...
# (oddaljen manj kot DIST_CLOSE), preden sprožimo varnostni mehanizem
...
PID_turn = PID(
    ...
    Ki=PID_TURN_KI,
    Kd=PID_TURN_KD,
    integral_limit=PID_TURN_INT_LIMIT)
...
PID_frwd_base = PID(
    ...
    Ki=PID_STRAIGHT_KI,
    Kd=PID_STRAIGHT_KD,
    integral_limit=PID_STRAIGHT_INT_LIMIT)
...
while do_main_loop and not btn.down:
    ...
    if game_state == -1:
        ...
        if game_on and robot_alive:
            ...
            speed_right = round(
                            min(
                                max(speed_right, -SPEED_LIMIT),
                                SPEED_LIMIT)
                            )
            speed_left = round(
                            min(
                                max(speed_left, -SPEED_LIMIT),
                                SPEED_LIMIT)
                            )
